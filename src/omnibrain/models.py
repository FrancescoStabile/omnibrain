...
class ApprovalGate:
    """Gate for actions requiring user approval.

    OmniBrain NEVER executes high-impact actions without explicit approval.
    ...
    def needs_approval(cls, tool_name: str) -> bool:
        ...
        if tool_name in cls.PRE_APPROVED:
            return False
        # Default: require approval for anything not explicitly pre-approved
        return True

    @classmethod
    def is_known_tool(cls, tool_name: str) -> bool:
        return tool_name in cls.PRE_APPROVED or tool_name in cls.REQUIRES_APPROVAL
